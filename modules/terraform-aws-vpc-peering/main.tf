...
resource "aws_route" "this_routes_region" {
  provider                  = aws.this
  count                     = length(var.this_vpc_rts)
  route_table_id            = tolist(var.this_vpc_rts)[count.index]
...
resource "aws_route" "peer_routes_region" {
  provider                  = aws.peer
  count                     = length(var.peer_vpc_rts)
  route_table_id            = tolist(var.peer_vpc_rts)[count.index]
